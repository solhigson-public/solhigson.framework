---
name: Cooperative Cancellation (.NET)
description: "CancellationToken propagation — named argument syntax requirement, async service/repository/controller signatures, linked CancellationTokenSource for Task.WhenAll"
scope: dotnet
tier: agent-injected
...
MUST ALWAYS pass `CancellationToken` when a called method accepts one. MUST NOT call an async method without CT when a CT-accepting overload exists (CA2016). Exception: fire-and-forget operations (background work dispatched via job queue, e.g., Hangfire) that MUST complete even if the request is cancelled — these do NOT accept CancellationToken.
