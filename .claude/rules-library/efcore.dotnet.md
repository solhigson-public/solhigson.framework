---
name: EF Core Data Layer
description: "Entity conventions (record types, IEfCoreGenIgnore), repository auto-generation, read/write patterns, archive-aware queries, migration safety"
scope: dotnet
tier: agent-injected
...
When creating a new entity or adding schema-affecting properties to an existing entity, MUST complete all three steps before committing:

1. **DbSet** — MUST add a `DbSet<T>` property in `AppDbContext.cs`. Without this, EF Core will not include the entity in migrations. `DbContext.Set<T>()` works at runtime for tracked entities but does NOT register the entity for migration scaffolding.
2. **Migration** — MUST run `dotnet ef migrations add <Name> --project src/<Infra> --startup-project src/<Web> --context AppDbContext` to scaffold the migration.
3. **Verify** — MUST read the generated migration `.cs` file to confirm it contains the expected schema changes (new table, new columns, indexes).

Schema-affecting changes that require a migration:
- New entity (new table)
- New property on an existing entity (new column)
- Changed property type, length, or nullability (column alteration)
- New or modified `[Index]` attributes
