---
name: Notification Pattern (.NET)
description: "NotifyAsync dispatch — fire-and-forget after SaveChangesAsync, try-catch wrapping, recipient lookup from DbContext.Users, UserNotificationRequest channel flags"
scope: dotnet
tier: agent-injected
...
MUST call `NotifyAsync` after `SaveChangesAsync()`. MUST wrap in try-catch — notification failure MUST NOT fail the business operation. If recipient lookup fails or `NotifyAsync` throws, MUST log the failure with context (user ID, notification type) and continue. Notifications are fire-and-forget. MUST look up recipient user info from `DbContext.Users`.
