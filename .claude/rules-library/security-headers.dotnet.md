---
name: Security Headers (.NET)
description: "Security header middleware — UseSecurityHeaders pipeline ordering, CSP nonce generation for inline scripts in Razor views, middleware registration before UseStaticFiles/UseRouting"
scope: dotnet
tier: agent-injected
...
MUST configure security headers in middleware (`UseSecurityHeaders()` or equivalent). MUST use CSP nonce generation for all inline scripts in Razor views. MUST register security header middleware before `UseStaticFiles()` and `UseRouting()` in the request pipeline.
