...
public class MongoDbService
{
    ...
    public async Task<T?> FindAsync<T>(string id) where T : IMongoDbDocumentBase
    {
        var coll = GetCollection<T>();
        if (coll is null)
        {
        ...
        return await coll.Find(doc => doc.Id == id).SingleOrDefaultAsync();
    }

    public async Task<List<T>> FindAsync<T>(Expression<Func<T, bool>> filter) where T : IMongoDbDocumentBase
    {
        var coll = GetCollection<T>();
        if (coll is null)
        {
    ...
    public async Task<PagedList<T>> FindAsync<T>(Expression<Func<T, bool>> filter, int pageNumber, int pageSize) where T : IMongoDbDocumentBase
    {
        var coll = GetCollection<T>();
        if (coll is null)
        {
        ...
        var count = await coll.Find(filter).CountDocumentsAsync();
        var result = await coll.Find(filter).Skip((pageNumber - 1) * pageSize).Limit(pageSize).ToListAsync();
        return PagedList.Create(result, count, pageNumber, pageSize);
    }

    public async Task UpdateAsync<T>(T document) where T : IMongoDbDocumentBase
    {
        var coll = GetCollection<T>();
        if (coll is null)
        {
        ...
        await coll.ReplaceOneAsync(sub => sub.Id == document.Id, document);
    }

    public async Task DeleteAsync<T>(string id) where T : IMongoDbDocumentBase
    {
        var coll = GetCollection<T>();
        if (coll is null)
        {
        ...
        await coll.DeleteOneAsync(sub => sub.Id == id);
    }
...
public class MongoDbService<T> where T : IMongoDbDocumentBase
{
    ...
    public async Task<T?> FindAsync(string id) => await _mongoDbService.FindAsync<T>(id);

    public async Task<List<T>> FindAsync(Expression<Func<T, bool>> filter) => await _mongoDbService.FindAsync(filter);
    
    public async Task<PagedList<T>> FindAsync(Expression<Func<T, bool>> filter, int pageNumber, int pageSize)
    {
        return await _mongoDbService.FindAsync(filter, pageNumber, pageSize);
    }

    public async Task UpdateAsync(T document) => await _mongoDbService.UpdateAsync(document);

    public async Task DeleteAsync(string id) => await _mongoDbService.DeleteAsync<T>(id);
}
